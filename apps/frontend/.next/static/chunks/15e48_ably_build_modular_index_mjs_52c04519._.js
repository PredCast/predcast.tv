...
"[project]/node_modules/.pnpm/ably@2.17.1_bufferutil@4.1.0_react-dom@19.2.4_react@19.2.4__react@19.2.4_utf-8-validate@6.0.6/node_modules/ably/build/modular/index.mjs [app-client] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
Released under the Apache Licence v2.0*/ __turbopack_context__.s({
    ...
    "XHRPolling": (()=>xhrpollingtransport_default),
    "XHRRequest": (()=>xhrrequest_default),
    "constructPresenceMessage": (()=>constructPresenceMessage),
    "decodeAnnotation": (()=>decodeAnnotation),
    "decodeAnnotations": (()=>decodeAnnotations),
    "decodeEncryptedMessage": (()=>decodeEncryptedMessage),
    "decodeEncryptedMessages": (()=>decodeEncryptedMessages),
    "decodeEncryptedPresenceMessage": (()=>decodeEncryptedPresenceMessage),
    "decodeEncryptedPresenceMessages": (()=>decodeEncryptedPresenceMessages),
    "decodeMessage": (()=>decodeMessage),
    "decodeMessages": (()=>decodeMessages),
    "decodePresenceMessage": (()=>decodePresenceMessage),
    "decodePresenceMessages": (()=>decodePresenceMessages),
    "generateRandomKey": (()=>generateRandomKey),
    "getDefaultCryptoParams": (()=>getDefaultCryptoParams)
});
...
var _Logger = class _Logger {
    constructor(){
      ...
      the object being serialised if the log level will not output the message */ this.shouldLog = (level)=>{
...
async function withTimeoutAsync(promise, timeout = 5e3, err = "Timeout expired") {
    ...
    return Promise.race([
        promise,
        new Promise((_resolve, reject)=>setTimeout(()=>reject(e), timeout))
    ]);
}
...
var Defaults = {
    ...
    TLS_PORT: 443,
    TIMEOUTS: {
        /* Documented as options params: */ disconnectedRetryTimeout: 15e3,
        suspendedRetryTimeout: 3e4,
        /* Undocumented, but part of the api and can be used by customers: */ httpRequestTimeout: 1e4,
        httpMaxRetryDuration: 15e3,
        channelRetryTimeout: 15e3,
        fallbackRetryTimeout: 6e5,
        /* For internal / test use only: */ connectionStateTtl: 12e4,
...
function getHosts(options) {
    ...
    ].concat(getFallbackHosts(options));
}
...
var Auth = class {
   ...
   * effect on the connection as #authorize does */ async _forceNewToken(tokenParams, authOptions) {
   ...
   * progress, making all pending callbacks wait for the new one */ async _ensureValidAuthCredentials(forceSupersede) {
    ...
    /* User-set: check types, '*' is disallowed, throw any errors */ _userSetClientId(clientId) {
    ...
    /* Ably-set: no typechecking, '*' is allowed but not set on this.clientId), return errors to the caller */ _uncheckedSetClientId(clientId) {
...
var Http = class {
    ...
    _getHosts(client) {
        ...
        if (connectionHost) {
            return [
                connectionHost
            ].concat(defaults_default.getFallbackHosts(client.options));
        }
...
var flags = {
    /* Channel attach state flags */ HAS_PRESENCE: 1 << 0,
    HAS_BACKLOG: 1 << 1,
    RESUMED: 1 << 2,
    TRANSIENT: 1 << 4,
    ATTACH_RESUME: 1 << 5,
    HAS_OBJECTS: 1 << 7,
    /* Channel mode flags */ PRESENCE: 1 << 16,
    PUBLISH: 1 << 17,
    SUBSCRIBE: 1 << 18,
...
var PaginatedResult = class {
   ...
   * the rest of a multipage set of results can always be done with GET */ async get(params) {
...
var ChannelSubscriptions = class _ChannelSubscriptions {
    constructor(client){
        /* ChannelSubscriptions have no unique id; removing one is equivalent to removeWhere by its properties */ this.remove = _ChannelSubscriptions.prototype.removeWhere;
        this.client = client;
    }
...
var RestChannel = class {
    ...
    async publish(...args) {
        ...
        if (typeof first === "string" || first === null) {
            ...
            messages = [
                message_default.fromValues(first)
            ];
            params = args[1];
...
var Channels = class {
   ...
   * Please do not use this unless you know what you're doing */ release(name) {
...
var RealtimeChannel = class _RealtimeChannel extends eventemitter_default {
    ...
    /* @returns null (if can safely be released) | ErrorInfo (if cannot) */ getReleaseErr() {
        const s = this.state;
...
function omitAgent(channelParams) {
    const _a2 = channelParams || {}, { agent: _ } = _a2, paramsWithoutAgent = __objRest(_a2, [
        "agent"
    ]);
...
var ConnectionManager = class _ConnectionManager extends eventemitter_default {
   ...
   * being activated */ noTransportsScheduledForActivation() {
   ...
   * server returns 403. */ actOnErrorFromAuthorize(err) {
        if (err.code === 40171) {
            this.notifyState({
                state: "failed",
                error: err
...
var Channels2 = class extends eventemitter_default {
   ...
   * for channels that are attaching or detaching. */ onTransportActive() {
   ...
   * attached, pending, or will attempt to become attached in the future */ propogateConnectionInterruption(connectionState, reason) {
...
var isPrime;
var factor;
var LittleEndian = !!new uint8Array(new uint32Array([
    1
]).buffer)[0];
function convertEndian(word) {
    if (LittleEndian) {
        return(// byte 1 -> byte 4
...
var BufferUtils = class {
   ...
   * to utf8-decode a string to another string is almost certainly a mistake */ utf8Decode(buffer) {
...
var createCryptoClass = function(config, bufferUtils) {
    ...
    class CBCCipher {
        ...
        async encrypt(plaintext) {
            ...
            const cryptoKey = await crypto.subtle.importKey("raw", this.key, this.webCryptoAlgorithm, false, [
                "encrypt"
            ]);
...
var FilteredSubscriptions = class {
    ...
    static addFilteredSubscription(channel, filter, realListener, filteredListener) {
        ...
        if (channel.filteredSubscriptions.has(realListener)) {
            ...
            realListenerMap.set(filter, ((_a2 = realListenerMap == null ? void 0 : realListenerMap.get(filter)) == null ? void 0 : _a2.concat(filteredListener)) || [
                filteredListener
            ]);
        } else {
            channel.filteredSubscriptions.set(realListener, /* @__PURE__ */ new Map([
                [
                    filter,
                    [
                        filteredListener
...
//# sourceMappingURL=15e48_ably_build_modular_index_mjs_52c04519._.js.map