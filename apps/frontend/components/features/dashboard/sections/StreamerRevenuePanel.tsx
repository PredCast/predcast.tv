...
interface StreamerRevenuePanelProps {
    readonly wallet: `0x${string}` | undefined;
}

export function StreamerRevenuePanel({ wallet }: StreamerRevenuePanelProps) {
    const sw = useStreamWallet({ streamerAddress: wallet });
    const { assetDecimals } = usePoolDecimals();
    // Donations *received* by this streamer (their wallet is the recipient).
    const donations = useStreamerDonations(wallet ?? '');

    // `useStreamWallet.statistics` returns already-formatted USDC strings.
    ...
    const donationsCount = donations.data?.donations?.length ?? 0;
    const isEmpty = totalRevenue === 0 && donationsCount === 0;
    ...
    if (isEmpty) {
        return (
            <section className="relative z-[4] mx-auto max-w-[1400px] px-8 pb-4 pt-20 sm:px-14">
                ...
                />
                <EmptyCard
                    icon={EMPTY_ICONS.satellite}
                    title="No revenue yet"
                    lead="Once your StreamWallet is deployed, donations and subscription payouts will appear here in real time. Withdraw to your wallet anytime."
                    tip="StreamWallet · per-streamer ERC-1967 proxy"
                />
            </section>
        );
    }
            ...
            <SectionHeadDash
                ...
                right={
                    <button
                        ...
                        disabled={txInFlight || available <= 0}
                        ...
                        style={{ boxShadow: txInFlight || available <= 0 ? 'none' : '0 8px 24px rgba(232,0,29,0.25)' }}
                    >
                ...
                <div className="rounded-xl border border-[#1E1E1E] bg-[#111] p-7">
                    ...
                    <div className="mt-6 grid grid-cols-3 gap-4 border-t border-[#1E1E1E] pt-6">
                        <div>
                            ...
                            <div className="font-display mt-2 text-[20px] font-bold leading-none tracking-[-0.01em]" style={{ color: '#2dd4a4' }}>
                                {fmtUsd(available)}
                            </div>
                        </div>
                        <div>
                    ...
                    {(donations.data?.donations?.length ?? 0) === 0 ? (
                        ...
                        <div className="flex flex-col">
                            {(donations.data?.donations ?? []).slice(0, 5).map((d) => {
                                ...
                                return (
                                    <div key={d.transactionHash} className="flex items-center justify-between gap-4 border-b border-[#1E1E1E] py-3 last:border-0">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="font-display text-[18px] font-extrabold leading-none tracking-[-0.01em]" style={{ color: '#2dd4a4' }}>
                                                +{fmtUsd(amount, { dp: 0 })}
                                            </div>
                                        </div>
                                    </div>
