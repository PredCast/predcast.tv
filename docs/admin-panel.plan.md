...
- Auth front : mêmes providers Dynamic Labs (copie de `DynamicProviderWrapper`/`auth-provider`), puis `AdminGuard` à machine d'états (discriminated union) : `wallet_disconnected → authenticating → checking_admin (GET /admin/me) → denied | error | admin`. `AdminSessionProvider` expose `{wallet, role}` ; `SidebarNav` et `RequireRole` filtrent par rôle (cosmétique — le backend re-vérifie tout).
